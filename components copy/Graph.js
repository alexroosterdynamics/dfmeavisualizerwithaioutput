...
export default function Graph({
  ...
  const handleNodeClick = (node, event) => {
    ...
    if (event && event.shiftKey) {
      // toggle node as extra source (strictly bounded by maxDepth)
  ...
  return (
    <div className="w-full h-[calc(100vh-57px)] relative bg-slate-900">
      <ForceGraph2D
        ...
        // linkCurvature={0.06} // optional: slight curve to separate overlaps
      ...
      <div className="absolute bottom-3 left-3 bg-slate-800/80 backdrop-blur border border-slate-700 rounded p-2 text-xs text-slate-200">
        ...
        <div className="mt-1 text-slate-400">
          Click = focus root. Shift+Click = add/remove as extra source. Depth is a hard cap.
        </div>
      </div>
    </div>
  );
